...
function requestPlaylistUpdate() {
  socket.emit('request_playlist_update', {
    room_id: currentRoom
  });
}
...
function fetchSuggest(_x) {
  return _fetchSuggest.apply(this, arguments);
}
function _fetchSuggest() {
  _fetchSuggest = _asyncToGenerator(function* (term) {
    try {
      const resp = yield fetch(`/api/bili/suggest?term=${encodeURIComponent(term)}`);
...
function _doSearch() {
  _doSearch = _asyncToGenerator(function* (page = 1) {
    ...
    document.getElementById('bili-detail-panel').style.display = 'none';
    try {
      const resp = yield fetch('/api/bili/search', {
        method: 'POST',
        headers: {
...
function onVideoResultClick(_x2) {
  return _onVideoResultClick.apply(this, arguments);
}
function _onVideoResultClick() {
  _onVideoResultClick = _asyncToGenerator(function* (bvid) {
    ...
    try {
      const resp = yield fetch(`/api/bili/pages?bvid=${encodeURIComponent(bvid)}`);
      const data = yield resp.json();
      if (data.error) {
